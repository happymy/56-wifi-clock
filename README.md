...
> **硬件来源**：本项目的硬件平台（STC15W408AS + TM1639 数码管 + DS1302 + ESP-01S 的「56」WiFi 时钟）购自 [56dz 数码之家](https://www.56dz.com/p/3759.html)，本文档所有引脚定义、器件连接与显示位序均依实物测绘验证（见 `plan/原理图.md`）。

> **项目演变**：立项时目标为**复刻官方固件功能**——以「56」WiFi 时钟原版规格（`doc/原版时钟功能.md`）为基准，复刻 51 固件全套功能（三态显示、亮度、温度补偿、三组闹钟、手动/联网对时、AP 配网）。**复刻推进到约一半、原版核心功能落地后，决定在原版之上加入更多功能**（手动 UP 整屏三态、°C/°F、Web 温度单点标定、计时器、8266 权威倒计时与显示接管、伪待机等，见 `plan/新版时钟功能.md` v2 规格）——即"先复刻、后扩展"两步走，本仓库实现为扩展后的版本。

硬件：STC15W408AS（1T 8051）+ TM1639 数码管驱动 + DS1302 实时时钟 + ESP-01S(ESP8266) WiFi。51 端固件按 v2 规格实现（复刻原版 + 新增功能），8266 端为自写固件（Web 配网/校时/倒计时权威）。
...
## 硬件概览
  ...
  - **CODE 容量已探测确认 = 8KB（8192B）**：5KB EEPROM 为**数据空间**，芯片不能从中取指执行，故无法用于缩减 CODE；仅作 M11 配置持久化。固件区上限 **8192B（8KB Flash）**，当前构建末端 **0x1FA1（8097B）**，余 95B（用户环境约 +4B → 8101 仍过 8192，零错误零告警，SDCC `--opt-code-size`）。计时器（51 原生）已实装、倒计时显示接管移 8266（见 `plan/新版时钟功能.md` §四/§五/§十八 容量取舍）；亮度主用 TM1639 自带 8 档硬件占空比（自动/手动）；极暗软件 PWM（≈1/1024）仅在 `demo/clock-bringup` 验证，**产品固件未移植**（见 `plan/新版时钟功能.md` §十九）。
- **显示**：TM1639 三线驱动 8 位数码管（共阴）：
  ...
  - 下排 `SMG1` → GRID5/6（`5612B` 双位）：TIME 模式由 `smg1_mode`（8266 下发，偏移21）固定选显 **温度 °C / 日期(日 DD)**（0=温度、1=日期，不轮换）；DATE 显星期 0–6（0=周日）；TEMP 灭。**SMG1 凡显温度个位(GRID5)小数点恒亮**。
...
用户使用手册.md    面向最终用户：刷固件 / 使用说明 / 排故手册
...
plan/             设计文档（权威依据）
  ...
  固件复刻计划.md       复刻路线与里程碑（先复刻后扩展，v2 为权威）
  新版时钟功能.md        51 产品固件 v2 规格/容量取舍/温度补偿（权威）
...
- ✅ **传统手动设置**（v0.5，2026-08-31 更新字段序/星期约定）：SET 单击进入并循环切换字段（年→月→日→星期→时→分→秒，星期 0–6 以 0=周日且 SMG1 左显数值右管空），UP 在当前字段加值（独立回绕、闰年感知、月 12→1 不设 0），末字段 SET 保存+响蜂鸣。
...
- ✅ **51 固件研发结束（v1.0.7）**：功能与硬件实机测试全部完成——串口协议帧（SET_CFG/对时/自动轮显/闹钟贪睡/计时器/°F 换算/日期/温度）逐项实机验证通过；`temp_offset` 单位（整数°C）实测标定；`SET_CFG` 读回防清零修复。51 端开发**收尾结束**，后续仅有随 8266 联调的排障性微调，不再有新功能开发。CODE 8097/8192B（余 95B）。
...
- ✅ **初始版本完成（v1.1）**：软件全部就绪（51 固件 + 8266 固件 + 《用户使用手册》），**进入长期试用阶段**。试用期沿用 v1.0.7/V1.0 语义——不新增功能，仅做持续运行的稳定性观察与随 8266 联调的排障性修复；发现 bug 时以 x.y.z 递增 minor 发布修复版。
...
| v1.0.7 / wifi-clock-51-v1.0.7 | **51 固件研发收尾**：实机测试全部通过（串口协议/闹钟贪睡/计时器/°F/日期/温度/自动轮显），`temp_offset` 实测标定（整数°C），SET_CFG 读回防清零修复；精简 test 目录+README、实测 CODE 8097/8192B（余 95B）并统一全文档。**51 端开发结束**，无新功能开发，后续仅 8266 联调排障 |
| v1.1 | **初始版本完成**（新增《用户使用手册》：刷固件/使用说明/排故），开始长期试用阶段 |
| v1.1.1 | **8266 排障修复**：配网后访问 `/wifi` 不再清空 WiFi 凭据（该路由仅响应 POST）；Web 配置页"状态灯"标签明确为红色 LED（避免与 ESP 侧蓝灯混淆）；**配网页提示增强**——配网失败后自动重开热点，页面据此显示红色"上次配网失败，请仔细检查"警告（有已存凭据却开 AP = 上次失败，无需新增状态），并补全"双击 SET 查看 IP / 重配网 = SET+UP 5 秒"等用户提示 |
| v1.1.2 | **8266 配网扫描修复 + 配网页重做**：修复冷启动（未配过的设备）进配网页扫不到网络的问题——根因是 `open_ap` 从未启用 STA 接口、不满足 `scanNetworks` 前置（需 STA 已启用+未连接）；`open_ap` 改 `WIFI_AP_STA` 双模 + `enableSTA(true)+disconnect()+delay` 后开软 AP（固定信道 6，防 STA 扫描拖走射频致客户端掉线）。配网页从 datalist 下拉改为**可见可点的 WiFi 列表**（SSID 左对齐、蓝色信号条、悬停高亮、点击自动填入、响应式适配手机），跨浏览器必显示，不再受 datalist 手机兼容影响；并加 `Cache-Control: no-store` 防止浏览器缓存旧版配网页（此前"下拉空"多为缓存所致）。删除"点击下拉选择，或直接输入"提示与 datalist，输入框回归纯手输+列表点选 |
| v1.1.3 | **Web 配置修复 + 配网页注入加固**：修复 STA 配置页「时区」下拉**长期失效**——`h_sta_root` 渲染了时区控件但 `h_sta_save` 从未写入 `g_cfg[13]`，改时区不生效（NTP 一直用旧时区）；修复后回填与默认改用有效时区 `wifi_tz_h()`（顺带修掉未同步前误显 0 而非默认 +8 的问题）。**安全加固**：AP 配网页扫描到的 SSID（可被伪造）此前仅过滤单引号，含 `" < > &` 的恶意 SSID 可注入页面脚本/破坏 HTML；新增 `&<>"'` 全量 HTML 转义（`esc()`），替换脆弱的单引号过滤。另将 51 无应答时"改动已暂存本机"的误导提示改为如实说明"未能下推，上线后重试"（离线路径本机既未持久化、也会被下次 REQ_CFG 覆盖） |
