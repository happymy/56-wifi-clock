...
## 硬件概览
  ...
  - **CODE 容量已探测确认 = 8KB（8192B）**：5KB EEPROM 为**数据空间**，芯片不能从中取指执行，故无法用于缩减 CODE；仅作 M11 配置持久化。固件编译占用 **8101/8192B**（含 M11 配置持久化），DSEG 余 162B，零错误零告警（SDCC `--opt-code-size`）。亮度用 TM1639 自带 8 档硬件占空比（自动/手动），未采用占用 T0 中断的软件 PWM 以保 8KB 容量。
- **显示**：TM1639 三线驱动 8 位数码管（共阴）：
  - 主行 4 大管 LED2–5 → GRID1–GRID4（`FJ12101AH` 1.2"）：时间模式 `HH:MM`、日期模式 `MMDD`（月左两位 + 日右两位镜像）、温度模式显温度值
