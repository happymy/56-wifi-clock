...
__xdata unsigned char g_ds_init_action = 0;   /* 0=保住已走时 1=CH=1掉电(电池未保住) 2=BCD非法；供调试 */
...
void ds1302_read_time(__xdata ds_time *t) {
    unsigned char i, k, b;
    __xdata unsigned char *p = (__xdata unsigned char *)t;
    P1 &= ~DS_SCLK;             /* DS1302：CE 上升沿前 SCLK 须为低，否则移位错位(读出乱码) */
    P1 |= DS_CE;
    ds_delay();
...
void ds1302_write_time(const __xdata ds_time *t) {
    ds_write_byte(0x8E, 0x00);   /* 解除写保护 */
