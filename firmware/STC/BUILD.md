...
> 适用：STC15W408AS 产品固件（51 端）空间极度紧张——CODE 上限 8192B，当前构建末端 8097B（0x1FA1，解析 `out/firmware.ihx` 实测），**余量 95B**。任何让体积膨胀的改动都可能溢出。本文件固定编译环境、尺寸检查手法与省体积红线，避免"换台机器/改个选项就烧不进去"。
...
本机 SDCC 4.5.0 构建末端当前 8097B（0x1FA1，余 95B）。但**另一套环境（不同 SDCC 版本、或 STC-ISP 打包链路差异）实测会多产出 ~4 字节**。因此：

- 本机构建末端 8097B（0x1FA1，余 95B），用户环境约 +4B → 8101，仍过 8192 且**余 91B**；建议保持 **≤ 0x1FF2（8178B）/ 余 ≥ 14B** 以内，给环境差量留缓冲。
