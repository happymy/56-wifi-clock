...
> 适用：STC15W408AS 产品固件（51 端）空间极度紧张——CODE 上限 8192B，当前构建末端 8181B（0x1FF5，解析 `out/firmware.ihx` 实测），**余量仅约 11B**。任何让体积膨胀的改动都可能溢出。本文件固定编译环境、尺寸检查手法与省体积红线，避免"换台机器/改个选项就烧不进去"。
...
本机 SDCC 4.5.0 构建末端约 8175B（0x1FEE）。但**另一套环境（不同 SDCC 版本、或 STC-ISP 打包链路差异）实测会多产出 ~4 字节**。因此：

- 本机构建末端当前 8175B（0x1FEE，余 17B），用户环境约 +4B → 8179 仍过 8192；建议保持 **≤ 0x1FEB（8171B）/ 余 ≥ 21B** 以内，给环境差量留缓冲。
